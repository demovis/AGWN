...
# !pip install -U torch-sparse -f https://pytorch-geometric.com/whl/torch-1.8.0+cu101.html
# !pip install -U torch-cluster -f https://pytorch-geometric.com/whl/torch-1.8.0+cu101.html
# !pip install -U torch-spline-conv -f https://pytorch-geometric.com/whl/torch-1.8.0+cu101.html
...
train_loader = DataLoader(data_list[:6000], batch_size=5,shuffle=True)
valid_loader = DataLoader(data_list[6000:8000], batch_size=5,shuffle=True)
test_data   =  DataLoader(data_list[8000:], batch_size=5,shuffle=True)
    ...
    for epoch in range(Epoch):
        ...
        with torch.no_grad():
            ...
            # MAPE = np.mean(np.abs((y - y_hat) / y)) * 100
                # torch.save(my_net.state_dict(), "GWNN_val_new.pkl")
...
if test ==True and plot==False:
    ...
    my_net.load_state_dict(torch.load("GWNN_val_new.pkl"))
